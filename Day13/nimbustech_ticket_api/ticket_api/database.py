...
declarative `Base` class that our ORM models inherit from.
...
# Resolve the project root (two levels up from this file) so the database
# path works regardless of where the app is launched from.
PROJECT_ROOT = Path(__file__).resolve().parent.parent

# SQLite database file, stored inside the `data` folder as `tickets.db`.
...
# Base is the parent class that all our ORM models (see orm.py)
